...
let renderhtml="";
    for ( each in parseddata){   //in is used to get the index value(each store the index value)
        ...
        renderhtml=renderhtml +  `<tr class="link">
                                <td><a  href="view.html?index=${each}"> ${parseddata[each].title}</a></td>
                                ...
                                <td><a href="templatelist.html?index=${each}"><button class="template" type="button" onclick="gettemplatepage(${each})">Templates</button></td></tr>`

       document.getElementById("listtable").innerHTML=renderhtml //the list id get from homepage inside ul tag
       
    }
}
function gettemplatepage(list_index){
...
function cancel(index){
    ...
    for (let each in parseddata){
        ...
        if (each!==index){
            newresume.push(parseddata[each])
        }
